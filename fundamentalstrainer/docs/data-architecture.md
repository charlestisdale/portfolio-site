...
3. **Lesson** — Source-learning unit, such as one Professor Messer video transcript.
...
data/imports/           Import audit records
...
Assessment items should have `generation.sourceKnowledgeIds` and `generation.sourceSeedIds`. If a question cannot trace back to a knowledge object, it should not be treated as platform content.
